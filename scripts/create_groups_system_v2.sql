...
CREATE TABLE IF NOT EXISTS workflow_permissions (
    ...
    target_id UUID, -- Can reference auth.users(id) for individual or groups(id) for group
    level_number INTEGER DEFAULT 1, -- For approvers, which level they can approve
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    
    -- Ensure proper assignment based on type
    CONSTRAINT check_assignment_consistency CHECK (
        (assignment_type = 'individual' AND target_id IS NOT NULL) OR
        (assignment_type = 'group' AND target_id IS NOT NULL) OR
        (assignment_type = 'all_non_approvers' AND target_id IS NULL)
    )
);
...
CREATE INDEX IF NOT EXISTS idx_workflow_permissions_target_id ON workflow_permissions(target_id);
...
CREATE TRIGGER update_workflow_permissions_updated_at BEFORE UPDATE ON workflow_permissions
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
